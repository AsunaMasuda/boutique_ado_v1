...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        """
        # storing pid, bag, save_info, billing_details, shipping_details, grand_total
        ...
        # Since stripe will store them as blank strings which is not the same as the null value we want in the database.
        for field, value in shipping_details.address.items():
            if value == "":
                shipping_details.address[field] = None

        # Update profile information if save_info was checked
        profile = None # Just so we know we can still allow anonymous users to checkout.
